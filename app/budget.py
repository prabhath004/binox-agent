"""Budget tracker — enforces hard limits on tokens, cost, chunks, and replans."""
...
@dataclass
class BudgetConfig:
    max_context_tokens_per_step: int = 800
...
class BudgetState:
    ...
    def estimated_cost(self) -> float:
        return (self.input_tokens / 1_000) * INPUT_COST_PER_1K + (self.output_tokens / 1_000) * OUTPUT_COST_PER_1K

    def to_dict(self) -> dict:
        return {
            "input_tokens": self.input_tokens, "output_tokens": self.output_tokens,
            "total_tokens": self.total_tokens, "estimated_cost_usd": round(self.estimated_cost, 6),
            "retrieved_chunks": self.retrieved_chunks, "replans_used": self.replans_used,
            "compression_events": self.compression_events, "events": self.events,
        }
...
class BudgetTracker:
    ...
    def record_llm_call(self, prompt_tokens: int, completion_tokens: int):
        ...
        self.state.events.append(f"llm_call: +{prompt_tokens} in / +{completion_tokens} out")
    ...
    def remaining_chunks(self) -> int:
        return max(0, self.config.max_retrieved_chunks - self.state.retrieved_chunks)
    ...
    def report(self) -> dict:
        return {
            **self.state.to_dict(),
            ...
            "budget_remaining_usd": round(self.config.max_cost_usd - self.state.estimated_cost, 6),
